...
アプリ起動時に `model.json` が見つかると TF.js モデル認識へ切り替わります。モデルが無い場合は、カメラフレームの縮小画素から簡易ハッシュを作る内部フォールバックで、ラベルと信頼度を自動的にシミュレートします。どちらの場合も操作は「カメラ起動 → 自動認識 → 生成」で統一され、UI上にデモ表記や手動ラベル選択はありません。

自動認識では表示中と異なる同一ラベルが4回連続した場合だけ結果を切り替え、同じラベルの信頼度変化も1回あたり1.5ポイント以内に抑えます。「認識更新」を押すと安定化をバイパスし、その瞬間のフレームから得た結果を即時確定します。

## 一時非表示の機能

実機確認フィードバックを受け、現在のUIでは NFC、URL/QR持ち帰り、画像ターゲットAR、レーザー彫刻向け導線を一時非表示にしています。コードとDOMは削除していません。再開時は `src/main.ts` の `deferredFeaturesEnabled` と、`index.html` の `data-feature="deferred"` / `hidden` を確認してください。
