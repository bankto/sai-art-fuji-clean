...
最終更新: 2026-07-11（M1〜M2.5 実機確認を優先）
...
| 5. 実装 | Cursor 指揮(Codex 実装 / Claude Code レビュー) | 完了(ソフト) | `apps/camera-ar-gomi-demo/` | M1〜M3 実装済み。M1〜M2.5の実機確認待ち。AR(.mind)・M3.5はユーザー判断で保留(2026-07-11) |
...
- [ ] 実機確認(M1〜M2.5): GitHub Pages HTTPS でカメラ・生成・URL/QR/NFC — **優先**(2026-07-11。ベース固め)
...
- 最後に成功した検証コマンド・日時:
- ブロッカー / 担当: なし
- 再開条件・次に実行する具体的操作: M1〜M2.5 実機確認(人間作業。GitHub Pages HTTPS でカメラ・生成・URL/QR/NFC)
...
指定値と実際に確認できた値を分ける。エイリアス解決・fallback後のモデルを確認できない場合は「不明」とする。20行を超える前に `docs/history/YYYY-MM.md` へ移す。
