...
- [x] 実機フィードバック反映(Codex): デモ手動選択の即時反映、デモ表示、カメラ画面フルスクリーン、操作オーバーレイ、作品画面調整、将来機能の一時非表示、上司向け説明資料を追加。lint/build成功(2026-07-12)
...
- ブロッカー / 担当: なし
- 再開条件・次に実行する具体的操作: ユーザー承認後にcommit/push/再デプロイし、人間が更新後URLをiPhone Safari / Android Chromeで確認する。現在はカメラ・生成を対象とし、URL/QR/NFC/AR/レーザー導線は一時非表示
