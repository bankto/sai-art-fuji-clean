...
- **モデル・effort**: 委譲のたびにタスクの難易度を見立て、`docs/workflows/orchestrate.md`「モデル・effort の自動判断」の対応表で選び、**委譲コマンドに毎回明示する**。Codex には `-c model_reasoning_effort=…` を毎回明示(モデルは CLI 推奨の既定)、Claude Code には `--model <haiku|sonnet|opus>` と `--effort <medium|high|xhigh>` を毎回明示する。**どちらも省略しない**。作業後は成果物と `docs/status.md` に「実行指定AI / モデル / effort」と「実行確認AI / モデル / effort」を分けて記録する。実モデルを確認できない場合は「不明」とし、指定値・エイリアスを実績として書かない
